...
`v0.1.0` runtime source asset 이름은 다음과 같습니다.
...
별도로 검증한 standalone installer도 사용할 수 있습니다. 옆에 완전한 source tree가 없으면 `ICA_ROUTER_REF`의 정확한 asset을 받습니다. 기본값은 `v0.1.0`입니다.
...
`ICA_ROUTER_REF`는 정확한 `vMAJOR.MINOR.PATCH` tag여야 합니다. 유효한 기존 `secrets.json` 또는 import 가능한 key-rotator 문서가 있을 때만 `ICA_ROUTER_NON_INTERACTIVE=1`을 설정하십시오. 그렇지 않으면 prompting 대신 fail-closed합니다. 이 설정이 없으면 secret 입력에 실제 terminal이 필요합니다.
